...
export default function Cookies() {
  ...
  return (
    <div className={`min-h-screen ${resolvedTheme === 'dark' ? 'dark' : ''}`}>
      <Head>
        <title>Cookies | FinNEXTHO</title>
        <meta name="description" content="Saiba como utilizamos cookies para melhorar sua experiência." />
      </Head>
        ...
        <main className="max-w-4xl mx-auto px-6 py-12">
          <div className={`${resolvedTheme === 'dark' ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-lg p-8`}>
            <div className="flex items-center mb-8">
              <FiDatabase className={`w-8 h-8 mr-3 ${resolvedTheme === 'dark' ? 'text-purple-400' : 'text-purple-600'}`} />
              <h1 className="text-3xl font-bold">Cookies</h1>
            </div>
            <div className="prose prose-lg max-w-none">
              <p className={`mb-6 ${resolvedTheme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>Utilizamos cookies para melhorar sua experiência, analisar o uso do site e personalizar conteúdo. Você pode gerenciar suas preferências de cookies nas configurações do navegador.</p>
              <ul className={`space-y-2 ${resolvedTheme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
                ...
                <li>• Saiba mais em nossa <Link href="/privacidade">Política de Privacidade</Link></li>
              </ul>
            </div>
          </div>
        </main>
